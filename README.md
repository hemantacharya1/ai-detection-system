...
**Detecting Nano-Banana–Generated Images using Provenance + CNN Forensics**
...
Image Input
   │
   ├── Layer A: Metadata / C2PA Provenance (Deterministic)
   │       ├─ If AI provenance found → AI (confidence = 1.0)
   │       └─ Else → fallback
   │
   └── Layer B: CNN-based Forensic Detection (Probabilistic)
           ├─ Patch-based ResNet-18 inference
           ├─ Aggregation across patches
           └─ Confidence score (AI vs Real)
```

## 🔍 Layer A — Metadata & Provenance Detection
...
- Looks for explicit AI-generation claims (e.g., DALL·E, Gemini, Firefly, diffusion models)
...
## 🧪 Layer B — CNN-based Forensic Detection
...
- **Input:** random image patches (224×224)
...
│   ├── layer_b_model.pt          # ResNet-18 trained weights
│   └── layer_b_config.json       # Inference configuration
│
├── dataset/                      # (Optional) Training data
│   ├── ai/
│   └── real/
│
├── download_real_images.py       # Unsplash data collection
├── generate_ai_images.py         # HF Nano-Banana dataset loader
├── layer_b_training.ipynb        # Training notebook
├── metadata_layer.py             # Layer A implementation
├── test.py                       # Local inference test
├── requirements.txt
└── README.md
```

## 🚀 Running Inference (No Training Required)
...
- Patch-level ROC-AUC ≈ 0.92
- Image-level ROC-AUC ≈ ~1.0 on provided test images
